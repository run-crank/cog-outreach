...
describe('Cog:GetManifest', () => {
  ...
  it('should return expected cog metadata', (done) => {
    ...
    cogUnderTest.getManifest(null, (err, manifest: CogManifest) => {
      expect(manifest.getName()).to.equal('automatoninc/outreach');
      expect(manifest.getLabel()).to.equal('Outreach');
  ...
  it('should return expected cog auth fields', (done) => {
    cogUnderTest.getManifest(null, (err, manifest: CogManifest) => {
      ...
      // RedirectUrl field
      const username: any = authFields.filter(a => a.key === 'redirectUrl')[0];
      ...
      // RefreshToken field
      const password: any = authFields.filter(a => a.key === 'refreshToken')[0];
  ...
  it('should return expected step definitions', (done) => {
    cogUnderTest.getManifest(null, (err, manifest: CogManifest) => {
      ...
      const hasProspectFieldEqualsStep: boolean = stepDefs.filter(s => s.getStepId() === 'ProspectFieldEqualsStep').length === 1;
      expect(hasProspectFieldEqualsStep).to.equal(true);
